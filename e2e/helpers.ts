...
export async function gotoWithMockedMaps(page: Page, mode: MockMapsMode = 'success'): Promise<void> {
  ...
  await page.goto('/')
  await page.waitForSelector('text=Paragliding Flight Viewer', { timeout: 15_000 })
}
...
export async function uploadSampleFlight(page: Page): Promise<void> {
  ...
  await page.waitForSelector('text=Paragliding Flight Viewer', { state: 'detached', timeout: 15_000 })
}
